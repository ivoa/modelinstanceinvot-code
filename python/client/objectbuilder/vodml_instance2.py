...
class VodmlInstance2(object):
    ...
    def proto_get_head_block(self, head_dmtype):
        ...
        for key, value in JsonBlockExtractor.search_join_container(self.json_instance).items():
            join_mapping = JsonBlockExtractor.search_subelement_by_id(
                self.json_view,
                value[Ele.JOIN][Att.dmref])
            print(value[Ele.JOIN][Att.dmref])
            print("============")
            DictUtils.print_pretty_json(join_mapping)
            self.join_iterators[key] = JoinIterator(value, join_mapping)
            
        print(self.votable_path)        
        votable = parse(self.votable_path)

        for key, value in self.join_iterators.items():
            print("a")
            for resource in votable.resources:
                print("b {}".format(resource.type))
                for table in resource.tables:
                    print(table.name + " " +  value.foreign_table)
                    if table.name == value.foreign_table:
                        value.connect_votable(table)
                        break 
        for key, value in self.join_iterators.items():
            print(primary_value.table_mapper._get_next_flatten_row())
