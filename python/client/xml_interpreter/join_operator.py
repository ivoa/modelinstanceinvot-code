...
class JoinOperator(object):
    ...
    def _set_filter(self):
        ...
        self.target_table_id = MappingBlockCursor.get_globals_collection(self.target_id)
        if self.target_table_id is not None:
            raise MappingException("Join with GLOBALS not implemented yet")
        
        for tableref in MappingBlockCursor.get_tablerefs():
            if MappingBlockCursor.get_templates_instance_by_dmid(tableref, self.target_id) is not None:
                self.foreign_xml_instance = MappingBlockCursor.get_templates_instance_by_dmid(tableref, self.target_id)
        ...
        for  ele in self.xml_join_block.xpath("//WHERE"):
            where = Where(ele.get("foreignkey"), ele.get("primarykey"))
            where.set_primary_col(self.table_ref)
            where.set_foreign_col(self.target_table_id )
            self.wheres.append(where)
        
        self.table_iterator = TableIterator(self.target_table_id, VOTablePointer.get_table((self.target_table_id)).to_table())
    
    def _set_foreign_instance(self):
        # TODO should be done once for ever
        index_map = VOTablePointer.get_id_index_mapping(self.target_table_id)
